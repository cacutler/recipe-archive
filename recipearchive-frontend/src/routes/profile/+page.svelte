<script lang="ts">
	...
	async function handleUpdateProfile() {
		...
		try {
			const updated: User = await apiClient.updateUser($authStore.user.id, {
				firstName,
				lastName,
				email
			});
	...
	<div class="profile-container">
		<div class="profile-card">
			<div class="header">
				<h1>My Profile</h1>
				{#if !editMode}
					<button on:click={() => (editMode = true)} class="btn btn-secondary">
						Edit Profile
					</button>
				{/if}
			</div>
			...
			{#if editMode}
				<form on:submit|preventDefault={handleUpdateProfile}>
					...
					<div class="form-actions">
						...
						<button type="button" on:click={cancelEdit} disabled={loading} class="btn btn-secondary">
							Cancel
						</button>
					</div>
				</form>
		...
		<div class="danger-zone">
			...
			<button on:click={handleDeleteAccount} disabled={loading} class="btn btn-danger-large">
				Delete Account
			</button>
		</div>
	</div>
